# SnapFrame - Stop Sharing Boring Screenshots
![promo-800x600](https://github.com/user-attachments/assets/ba72c7f1-638a-4d11-a9a3-4143bf457dc1)

## SnapFrame Feedback

This repository is for bug reports and feature requests for [SnapFrame](https://snapframe.app).

## How to Report

- 🐛 **Bug Report**: [Open an issue](../../issues/new?template=bug_report.md)
- 💡 **Feature Request**: [Open an issue](../../issues/new?template=feature_request.md)

## Links

- 🌐 Website: [snapframe.app](https://snapframe.app)
- 🐦 X/Twitter: [@snapframe_app](https://x.com/snapframe_app)
- 📧 Support: support@snapframe.app

